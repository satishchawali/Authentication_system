...
const ResetPassword = () => {
  ...
  return (
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-600 to-yellow-100'>
      ...
      {/* Enter email Id */}
      {
        ...
        <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <button className='w-full py-2.5 bg-gradient-to-r from-indigo-300 to-indigo-900 text-white rounded-full mt-3'>Submit</button>
        </form>
      }
        ...
        <form onSubmit={onSubmitOTP} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <div className='flex justify-between mb-8' onPaste={handlePaste}>
            {Array(6).fill(0).map((_, index) => (
              <input className='w-12 h-12 bg-[#333A5C] text-white text-center text-xl rounded-md' ref={e => inputRefs.current[index] = e} onInput={(e) => handleInput(e, index)} type="text" key={index} onKeyDown={(e) => handleKeyDown(e, index)} required maxLength={1} />
            ))}
          </div>
